...
install_dependencies() {
    ...
    # gdown via python3 -m pip (avoids PATH issues with pip-installed scripts)
    if python3 -c "import gdown" &>/dev/null 2>&1; then
        info "gdown already present -- skipping"
    else
        info "Installing gdown via pip..."
        (python3 -m pip install -q --upgrade gdown >> "$LOG_FILE" 2>&1) &
        spinner $! "pip install gdown"
        ok "gdown installed"
    fi

    # Add pip user-bin to PATH so gdown binary is reachable if needed
    export PATH="$PATH:/root/.local/bin:$(python3 -m site --user-base)/bin"
}
...
download_rar() {
    ...
    # Use python3 -m gdown (bypasses PATH issues) + --fuzzy for sharing URLs
    if ! python3 -m gdown --fuzzy "$GDRIVE_URL" -O "$RAR_FILE" 2>&1 | tee -a "$LOG_FILE"; then
...
run_start() {
    ...
    # ── Installing Acunetix heading ──────────────────────────
    echo ""
    echo -e "${BOLD}${BLUE}╔══════════════════════════════════════════════════════╗${RESET}"
    echo -e "${BOLD}${BLUE}║                                                      ║${RESET}"
    echo -e "${BOLD}${BLUE}║        🔧  INSTALLING ACUNETIX v24.1 …               ║${RESET}"
    echo -e "${BOLD}${BLUE}║                                                      ║${RESET}"
    echo -e "${BOLD}${BLUE}╚══════════════════════════════════════════════════════╝${RESET}"
    echo ""
...
wait_and_finish() {
    ...
    # ── Strip immutable flags on license files (chattr +i) ──
    echo ""
    info "Clearing immutable flags on license files..."
    local lic_paths=(
        "acunetix/.acunetix/data/license"
        "../acunetix/.acunetix/data/license"
        ".acunetix/data/license"
    )
    local found_lic=0
    for lic_dir in "${lic_paths[@]}"; do
        if [[ -d "$lic_dir" ]]; then
            chattr -i -R "$lic_dir" >> "$LOG_FILE" 2>&1 || true
            for f in "$lic_dir/license_info.json" "$lic_dir/wa_data.dat"; do
                if [[ -f "$f" ]]; then
                    chattr -i "$f" >> "$LOG_FILE" 2>&1 || true
                    rm -f "$f" \
                        && ok "Removed $f" \
                        || warn "Could not remove $f — try: sudo chattr -i $f"
                fi
            done
            ok "License dir unlocked: $lic_dir"
            found_lic=1
            break
        fi
    done
    [[ $found_lic -eq 0 ]] && info "License dir not found — skipping chattr step"

    # ── Activating heading ───────────────────────────────────
    echo ""
    echo -e "${BOLD}${GREEN}╔══════════════════════════════════════════════════════╗${RESET}"
    echo -e "${BOLD}${GREEN}║                                                      ║${RESET}"
    echo -e "${BOLD}${GREEN}║        ⚡  ACTIVATING ACUNETIX v24.1 ...              ║${RESET}"
    echo -e "${BOLD}${GREEN}║                                                      ║${RESET}"
    echo -e "${BOLD}${GREEN}╚══════════════════════════════════════════════════════╝${RESET}"
    echo ""
